...
def compute_rep_features(landmark_sequence: List[np.ndarray]) -> Dict[str, float]:
    ...
    # Spine angle (approximate via shoulder-hip-knee)
    spine = np.array([joint_angle(xyz[t, 11], xyz[t, 23], xyz[t, 25]) for t in range(T)])

    # Hip midpoint vertical displacement
    ...
    features = {
        ...
        "knee_angle_range":   float(max(knee_L.max() - knee_L.min(), knee_R.max() - knee_R.min())),

        # Hip
        "hip_angle_min":      float(min(hip_L.min(), hip_R.min())),
        "hip_angle_max":      float(max(hip_L.max(), hip_R.max())),

        # Elbow (relevant for OHP)
        ...
        # Symmetry (mean absolute L-R difference)
        ...
        "knee_temporal_var":  float(np.var(np.diff(knee_L))),
        "hip_temporal_var":   float(np.var(np.diff(hip_L))),

        # Velocity (average angular velocity of knee)
        "knee_avg_velocity":  float(np.mean(np.abs(np.diff(knee_L)))),
    }
